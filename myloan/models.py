...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    date_of_birth = models.CharField(
        max_length=30,  # Enough space for various date formats
        null=True, 
        blank=True,
        help_text="Format: DD/MM/YYYY or YYYY-MM-DD"
    )    
...
class Loan(models.Model):
    date_applied = models.DateField()  # or models.DateTimeField()
    date_applied = models.DateField()  # or models.DateTimeField()
...
class CentralBankOfLesotho(models.Model):
    LOAN_STATUS_CHOICES = [
        ('active', 'Active'),
        ('paid', 'Paid'),
        ('defaulted', 'Defaulted'),
    ...
    national_id = models.CharField(
        ...
        help_text="Lesotho national identification number"
    )
    loan_amount = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        verbose_name="Loan Amount (M)"
    )
    ...
    def __str__(self):
        return f"{self.first_name} {self.last_name} - M{self.loan_amount}"
    ...
    def save(self, *args, **kwargs):
        if not self.due_date:
            self.due_date = self.date_issued + timedelta(days=365)
        super().save(*args, **kwargs)
    ...
    def is_overdue(self):
        """Method instead of property for admin compatibility"""
        if self.status == 'active' and self.due_date:
            return timezone.now().date() > self.due_date
        return False
    is_overdue.boolean = True  # For nice admin display
    is_overdue.short_description = 'Is Overdue'
from django.db import models

class PaidAccounts(models.Model):
    loan = models.ForeignKey('Loan', on_delete=models.CASCADE, related_name='paid_accounts')
    recipient_email = models.EmailField(max_length=50)
    amount_lsl = models.DecimalField(max_digits=12, decimal_places=2)
    ...
    def save(self, *args, **kwargs):
        self.status = 'PAID'  # Hardcode status to 'PAID'
        super().save(*args, **kwargs)
    ...
    def __str__(self):
        return f"Payment for Loan #{self.loan_id} - {self.amount_lsl} LSL"